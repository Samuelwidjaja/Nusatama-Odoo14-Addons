...
class ApprovalMatrix(models.Model):
    ...
    company_id = fields.Many2one('res.company', string="Company", required=True, ondelete="restrict", default=False, domain=False)
    model_id = fields.Many2one('ir.model', string="Model", required=True, ondelete="cascade", default=False, domain=[])
    ...
    user_request_field_id = fields.Many2one('ir.model.fields', string="User Requester Field", ondelete="cascade")
...
class ApprovalMatrixRule(models.Model):
    ...
    matrix_id = fields.Many2one('approval.matrix', string="Matrix", required=True, ondelete="restrict", default=False, domain=[])
    ...
    field_id = fields.Many2one('ir.model.fields', string="Field", required=True, ondelete="cascade", default=False, domain=[])
...
class ApprovalMatrixApprover(models.Model):
    ...
    seq = fields.Integer(string="Level", required=True)
    matrix_id = fields.Many2one('approval.matrix', string="Matrix", required=True, ondelete="restrict", default=False, domain=[])
...
class ApprovalMatrixMixin(models.AbstractModel):
    ...
    def open_reject_message_wizard(self):
        ...
        context.update({'default_prefix_message':"<h4>%s Rejecting Document %s</h4>" % (self.env.user.login,self.display_name,),"default_suffix_action": "button_reject"}) #uncomment if need append context
        context.update({'active_id':self.id,'active_ids':self.ids,'active_model':self._name})
        res = {
            'name': "%s - %s" % (_('Rejecting %s') % (self._description,), self.name),
            'view_type': 'form',
            'view_mode': 'form',
    ...
    def open_approvals(self):
        ...
        action = self.env['ir.actions.act_window']._for_xml_id('approval_matrix.action_approval_matrix_document_readonly')
        action.update({
...
class ApprovalMatrixDocumentApproval(models.Model):
    ...
    approved_count = fields.Integer("Approved Count",
         compute="_compute_approval", compute_sudo=True
        # search="_search_approved"
    )
    rejected_count = fields.Integer("Rejected Count", compute="_compute_approval", compute_sudo=True)
    approved = fields.Boolean("Is Approved", compute="_compute_approval", store=True, compute_sudo=True)

    user_can_approve = fields.Boolean(string="User can approve", compute="_compute_user_can_approve")
    active = fields.Boolean("Active", default=True)


    # FIXME: ??IS REALLY NEED SEARCH APPROVED_COUNT??
    # def _search_approved(self, operator, values):
    #     domains = [('approver_ids',operator,value)]
    #     domains += [('res_model','=',self._name)]
    ...
    def _send_notification(self):
        for rec in self:
            msg = "<b>APPROVAL REQUEST for %s #%s</b><br/>DOC. %s need to review for your approval!" % (rec.res_model_id.name, rec.res_name, rec.res_name)
            if len(rec.approver_ids):
                partner = rec.approver_ids.mapped('partner_id')
                doc = self.env[rec.res_model].browse(rec.res_id)
                try:
                    # doc.message_notify(subject='APPROVAL REQUEST for %s #%s' % (rec.res_model_id.name, rec.res_name),
                    #     body=msg,
                    #     partner_ids=partner.ids,
                    #     subtype_id=self.env.ref('mail.mt_note').id,
                    #     email_layout_xmlid='mail.mail_notification_light',)
                    doc.message_post(
                        subject='APPROVAL REQUEST for %s #%s' % (rec.res_model_id.name, rec.res_name),
                        body=msg,
                        partner_ids=partner.ids,
                        values={'self': doc, 'origin': rec.res_name},
...
class MessagePostWizard(models.TransientModel):
    ...
    rejection_message_id = fields.Many2one('rejection.message', string="RejectionMessage")
    ...
    def confirm(self):
        ...
        if not Record.exists():
            raise UserError(_("Record doesn't exist!"))
